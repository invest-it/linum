...
bool Function(dynamic) newerThan(Timestamp timestamp) {
  return (dynamic a) => ((a as Map<String, dynamic>)["time"] as Timestamp).compareTo(timestamp) <= 0;
}
...
bool Function(dynamic) inBetween(
  ...
  return (dynamic a) =>
  ((a as Map<String, dynamic>)["time"] as Timestamp).compareTo(timestamp1) <= 0 ||
      (a["time"] as Timestamp).compareTo(timestamp2) >= 0;
}

bool Function(dynamic) amountMoreThan(num amount) {
  return (dynamic a) => ((a as Map<String, dynamic>)["amount"] as num).compareTo(amount) > 0;
}
